...
`Maybe`를 사용하기전에 우리는 이미 타입 매개변수를 가진 타입을 사용해왔습니다. 바로 리스트 타입입니다. 리스트 타입은 구체적인 타입을 생성하기 위해서 매개변수를 사용합니다. 리스트의 값들은 `[Int]`타입, `[Char]`타입, `[[String]]`타입을 가질 수 있지만, 타입이 `[]`뿐인 값을 가질 수는 없습니다.      
