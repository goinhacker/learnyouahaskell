...
**커링\(Currying\)이란 인자를 여러개 받는 함수를 분리하여, 인자를 하나 받는 함수의 체인으로 만드는 방법입니다. 하스켈에서 모든 함수는 일급함이고 curried되어 있기때문에, 모든 함수가 한개의 인자만 받을 수 있습니다.** 하스켈에서 여러개의 인자를 받는 모든 함수들은 실제로는 curried functions 입니다. 커링을 사용하면 **partial application**을 작성할 수 있도록 해주어 편리합니다. **Partial application은 커링으로 쪼개진 부분 함수들을 조립하여 어플리케이션을 작성하는 방식입니다.**
