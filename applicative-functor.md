...
예를들어, `(+) <$> Just 3 <*> Just 5`는 안에 값이 있거나 없을 수 있는 값을 매개변수로 `+`를 호출합니다. `(+) <$> (+10) <*> (+5)`는 `(+10)`과 `(+5)`의 미래 결과값들을 매개변수로 `+`를 호출합니다.
...
예를들어, `[(+3),(*2)] <*> [1,2]`에서 `(+3)`은 `1`과 `2`에 모두 적용되고, `(*2)`도 마찬가지로 모두 적용되어 4개의 결과값을 포함한 `[4,5,2,4]`가 됩니다.

다른 방법으로 `[(+3),(*2)] <*> [1,2]`에서 첫번째 함수는 첫번째값만, 두번째 함수는 두번째값만 적용해서 `[1 + 3, 2 * 2]`, 결과적으로 `[4, 4]`가되는 어플리케이티브 펑터가 바로 `ZipList`입니다.
