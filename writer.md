...
리스트는 반복적으로 붙여나갈때 비효율적으로 동작할 수 있기때문에, 항상 효율적으로 붙여지는 디퍼런스 리스트\(difference list\)를 고려할 수 있습니다. 디퍼런트 리스트는 리스트와 유사하지만, 일반적인 리스트가 아니고 리스트를 가져와서 다른 리스트에 붙이는 함수 입니다. 예를들어 `[1,2,3]`의 디퍼런스 리스트, `\xs -> [1,2,3] ++ xs`입니다. 일반적인 빈 리스트 `[]`가 디퍼런스 리스트에서는 `\xs -> [] ++ xs` 입니다.

디퍼런스 리스트의 장점은 항상 효율적으로 리스트를 붙일 수 있다는 점입니다. 두개의 일반적인 리스트를 `++`로 붙일때는 왼쪽에 있는 리스트의 끝까지 까서 다른 리스트를 붙여야 합니다. 반면에 디퍼런스 리스트는 두 리스트를 붙일때 아래와 같이 동작할 수 있습니다. 

```haskell
f `append` g = \xs -> f (g xs)  
```

여기서 `f`와 `g`는 리스트를 받아서 어떤 것을 거기에 붙이는 함수입니다. 예를들어 `f`가 `("dog"++)`\(다르게 표현하면 `\xs -> "dog" ++ xs`\) 이고, `g`가 `("meat"++)`이라면 ``f `append` g``는 아래와 같은 함수를 만듭니다. 

```haskell
\xs -> "dog" ++ ("meat" ++ xs)
```

먼저 하나의 디퍼런스 리스트를 다른 리스트에 적용한 다음 다른 리스트에 적용하는 새로운 함수를 만들어 두개의 디퍼런스 리스트를 붙였습니다. 

이제 디퍼런스 리스트를 위한 _newtype_ 래핑을 만들어 보겠습니다. 이렇게 하면 쉽게 모나드 인스턴스에 입력으로 넣을 수 있습니다. 

```haskell
newtype DiffList a = DiffList { getDiffList :: [a] -> [a] }
```

이 타입은 `[a] -> [a]`의 래핑입니다. 왜냐하면 디퍼런스 리스트는 단순히 어떤 리스트를 받아서 다른 리스트를 반환하는 함수이기 때문입니다. 일반적인 리스트를 디퍼런스 리스트로 바꾸는 것은 간단합니다. 
...
디퍼런스 리스트에 일반적인 리스트를 넣을때 리스트를 다른 리스트에 붙이는 함수로 바꾸어서 래핑합니다. 왜냐하면 디퍼런스 리스트는 어떤 리스트를 다른 리스트에 붙이는 함수이기 때문입니다. 만약 그 리스트를 그대로 얻길 원하면 빈 리스트를 적용하면 됩니다. 
...
`mempty`는 항등 함수이고 `mappend`는 단순히 함수를 합성합니다. 실제 동작을 살펴보겠습니다.
...
이제 `gcdReverse` 함수를 일반 리스트 대신, 디퍼런스 리스트를 사용해서 효율적으로 동작하도록 할 수 있습니다.
...
모노이드를 `[String]`에서 `DiffList String`으로 바꾸고 `tell` 함수에서 일반 리스트를 `toDiffList`로 디퍼런스 리스트에 넣었습니다. 이제 로그와 함 제대로 동작하는지 확인해보겠습니다.
