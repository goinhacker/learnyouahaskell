...
이전 챕터에서 만들었던 `Tree a`도 `Functor`로 만들어질 수 있습니다. `Tree`도 비어있거나 다양한 타입이 들어갈 수 있는 상자가 될 수 있습니다. 그리고 `Tree`의 타입 생성자는 정확히 하나의 타입 파라메터를 받습니다. 만약 `fmap`를 `Tree`만 적용이 가능한 함수로 만들면, `(a -> b) -> Tree a -> Tree b`가 됩니다. 
